...
if ($product->is_in_stock()) : ?>
    ...
    <form class="buy-now cart" method="post" enctype='multipart/form-data' data-product_id="<?php echo esc_attr($post->ID); ?>">
        ...
        <button type="submit" class="single_add_to_cart_button button alt">
            <?php echo apply_filters('single_add_to_cart_text', sprintf(__('Buy Now %s', 'ultimate-woocommerce-auction-custom'), wc_price($product->get_regular_price())), $product); ?></button>
        <div>
            <input type="hidden" name="add-to-cart" value="<?php echo esc_attr($product->get_id()); ?>" />
