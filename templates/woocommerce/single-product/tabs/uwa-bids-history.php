...
<?php if (($product->is_woo_ua_closed() === TRUE) and ($product->is_woo_ua_started() === TRUE)) : ?>
	...
	<?php if ($product->get_woo_ua_auction_fail_reason() == '1') {
		_e('Auction Expired because there were no bids', 'ultimate-woocommerce-auction-custom');
	} elseif ($product->get_woo_ua_auction_fail_reason() == '2') {
		_e('Auction expired without reaching reserve price', 'ultimate-woocommerce-auction-custom');
	}

	if ($product->get_woo_ua_auction_closed() == '3') { ?>
		<p><?php _e('Product sold for buy now price', 'ultimate-woocommerce-auction-custom') ?>: <span><?php echo wc_price($product->get_regular_price()) ?></span></p>
	<?php } elseif ($product->get_woo_ua_auction_current_bider()) { ?>
		<p><?php _e('Highest bidder was', 'ultimate-woocommerce-auction-custom') ?>: <span><?php echo esc_attr(uwa_user_display_name($current_bidder)); ?></span></p>
	<?php } ?>
<?php endif; ?>
<table id="auction-history-table-<?php echo esc_attr($product->get_id()); ?>" class="auction-history-table">
	...
	<tr class="start">
		...
		<?php if ($product->is_woo_ua_started() === TRUE) { ?>
			<td class="started"><?php esc_html_e('Auction started', 'ultimate-woocommerce-auction-custom'); ?>
			<?php } else { ?>
			<td class="started"><?php esc_html_e('Auction starting', 'ultimate-woocommerce-auction-custom'); ?>
			<?php } ?></td>
