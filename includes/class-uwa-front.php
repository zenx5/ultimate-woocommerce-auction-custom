...
class UWA_Front
{
	...
	public function __construct()
	{
		...
		//Auction Type
		$this->uwa_types =  array('normal' => __('Normal', 'ultimate-woocommerce-auction-custom'), 'reverse' => __('Reverse', 'ultimate-woocommerce-auction-custom'));
		...
		if (!is_admin()) {
			...
			if (!empty($_GET['pay-uwa-auction'])) {
				...
				if ($current_user->ID != $product_data->get_woo_ua_auction_current_bider()) {
					wc_add_notice(sprintf(__('You can not buy this auction because you have not won it!', 'ultimate-woocommerce-auction-custom'), $product_data->get_title()), 'error');
					return false;
				}
		...
		if (is_user_logged_in()) {
			...
			if (method_exists($product, 'get_type') && $product->get_type() == 'auction') {
				...
				if ($user_id == $product->get_woo_ua_auction_current_bider() && !$product->get_woo_ua_auction_closed()) { ?>
					...
					<span class="uwa_winning" data-auction_id="<?php echo esc_attr($product->get_id()); ?>" data-user_id="<?php echo esc_attr(get_current_user_id()); ?>"><?php _e('Winning!', 'ultimate-woocommerce-auction-custom'); ?></span>
	...
	public function uwa_auction_private_msg_tab($tabs)
	{
		...
		if (method_exists($product, 'get_type') && $product->get_type() == 'auction') {
			...
			$tabs['uwa_auction_private_msg_tab'] = array(
				'title' => __('Private message', 'ultimate-woocommerce-auction-custom'),
				'priority' => 50,
	...
	function send_private_message_process_ajax()
	{
		...
		if (empty($firstname)) {
			$response['status'] = 0;
			$response['error_name'] = __('Please enter your Name!', 'ultimate-woocommerce-auction-custom');
			$sending = 0;
		}
		if (!is_email($email_id) || empty($email_id)) {
			$response['status'] = 0;
			$response['error_email'] = __('Please enter your Email address!', 'ultimate-woocommerce-auction-custom');
			$sending = 0;
		}
		if (empty($message)) {
			$response['status'] = 0;
			$response['error_message'] = __('Please enter a message!', 'ultimate-woocommerce-auction-custom');
			$sending = 0;
		}
		...
		if ($sending == 1) {
			...
			$response['status'] = 1;
			$response['success_message'] = __('Thank you for Contact.', 'ultimate-woocommerce-auction-custom');
		}
	...
	function uwa_ajax_watchlist_auction()
	{
		...
		if (is_user_logged_in()) {
			...
			<p>
				<?php
				printf(__('<span class="watchlist-error">Please Login/Register in to add auction to watchlist. </span><a href="%s" class="button watchlist-error">Login/Register &rarr;</a>', 'ultimate-woocommerce-auction-custom'), get_permalink(wc_get_page_id('myaccount')));
				?>
			</p>
<?php }
	...
	function uwa_ajax_finish_auction_fun()
	{
		...
		if (isset($_POST["post_id"])) {
			...
			if ($product_data->is_woo_ua_closed()) {
				...
				if (isset($_POST["ret"]) && $_POST["ret"] != '0') {
					...
					if ($product_data->is_woo_ua_reserved()) {
						if (!$product_data->is_woo_ua_reserve_met()) {
							...
							_e("Reserve price has not been met!", 'ultimate-woocommerce-auction-custom');
							echo "</p>";
							die();
						}
					}
					...
					if ($current_bidder) {
						...
						printf(__("Winning bid is %s by %s.", 'ultimate-woocommerce-auction-custom'), wc_price($product_data->get_woo_ua_current_bid()), uwa_user_display_name($current_bidder));
						echo "</p>";
						if (get_current_user_id() == $product_data->get_woo_ua_auction_current_bider()) {
							echo '<p><a href="' . apply_filters('ultimate_woocommerce_auction_pay_now_button_text', esc_attr(add_query_arg("pay-uwa-auction", $product_data->get_id(), woo_ua_auction_get_checkout_url()))) . '" class="button">' . __('Pay Now', 'ultimate-woocommerce-auction-custom') . '</a></p>';
						}
					} else {
						echo "<p>";
						_e("There were no bids for this auction.", 'ultimate-woocommerce-auction-custom');
						echo "</p>";
						die();
					}
				}
				...
				printf(__("Please refresh page.", 'ultimate-woocommerce-auction-custom'));
				...
				foreach ($posts_ids as $posts_id) {
					...
					if ($product_data->is_woo_ua_reserved() === TRUE) {
						if ($product_data->is_woo_ua_reserve_met() === FALSE) {
							$response[$posts_id]['wua_reserve'] = __("Reserve price has not been met.", 'ultimate-woocommerce-auction-custom');
						} elseif ($product_data->is_woo_ua_reserve_met() === TRUE) {
							$response[$posts_id]['wua_reserve'] = __("Reserve price has been met.", 'ultimate-woocommerce-auction-custom');
						}
					}
				}
			}
