...
function uwa_bid_place_message($product_id)
{
	...
	if ($current_user->ID == $product_data->get_woo_ua_auction_current_bider()) {
		...
		if (!$product_data->is_woo_ua_reserve_met()) {
			...
			$message = sprintf(__('Your Bid Placed Successfully for &quot;%s&quot;.', 'ultimate-woocommerce-auction-custom'), $product_data->get_title());
		} else {
			if ($product_data->get_woo_ua_auction_max_bid()) {

				$message = sprintf(__('Your Bid has been placed successfully for &quot;%s&quot;! Your max bid is %s.', 'ultimate-woocommerce-auction-custom'), $product_data->get_title(), wc_price($product_data->get_woo_ua_auction_max_bid()));
			} else {
				$message = sprintf(__('Your Bid has been placed successfully for &quot;%s&quot;!', 'ultimate-woocommerce-auction-custom'), $product_data->get_title());
			}
		}
	} else {
		$message = sprintf(__("Your bid has been placed successfully but you have been outbid for &quot;%s&quot;!", 'ultimate-woocommerce-auction-custom'), $product_data->get_title());
	}
...
function uwa_see_more_bids_ajax_callback()
{
	...
	if (sanitize_key($_POST['show_rows'] == -1)) {
		...
		$response['uwa_label_text'] = __('See less.', 'ultimate-woocommerce-auction-custom');
	} else {
		...
		$response['uwa_label_text'] = __('See more', 'ultimate-woocommerce-auction-custom');
	}
