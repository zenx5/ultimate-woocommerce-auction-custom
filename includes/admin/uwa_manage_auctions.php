...
class Woo_Ua_Logs_List_Table extends WP_List_Table
{
	...
	public function uwa_auction_get_data($per_page, $page_number)
	{
		...
		foreach ($auction_item_array as $single_auction) {
			...
			$query_bidders = $wpdb->prepare(
				"SELECT * FROM $tbl_log WHERE auction_id = %d ORDER BY id DESC LIMIT 2",
				$single_auction->ID
			);
			...
			if (!empty($results)) {
				...
				$tbl_log = $wpdb->prefix . 'woo_ua_auction_log';
				$query_bidders_count = $wpdb->prepare(
					"SELECT * FROM $tbl_log WHERE auction_id = %d ORDER BY id DESC",
					$single_auction->ID
				);
	...
	function prepare_items()
	{
		...
		$this->items = $this->uwa_auction_sort_array($this->uwa_auction_get_data(
			$per_page,
			$current_page
		));
	}
